...
import { CNPJ_MASK_ALFANUMERICO, unmaskCnpjAlfanumerico } from '../../masks/masks';
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => CnpjFormControlComponent),
      multi: true,
    },
  ],
  template: `
...
export class CnpjFormControlComponent extends BaseFormControlComponent implements ControlValueAccessor {
  readonly cnpjMask = CNPJ_MASK_ALFANUMERICO;
  ...
  // AA.AAA.AAA/AAAA-DD = 18 characters
  isFull = (el: HTMLInputElement) => el.value.length >= 18;

  private toDisplayValue(value: string): string {
    return maskitoTransform(value, this.cnpjMask);
  }

  private toModelValue(display: string): string | null {
    return unmaskCnpjAlfanumerico(display) || null;
  }
}
